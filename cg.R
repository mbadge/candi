...
(regex_inpt_type_id <- capture(one_or_more(ALPHA)) %R%
     "Input" %R% OPEN_PAREN %R% or("'", '"') %R%
     capture(one_or_more(WRD)) %R% or("'", '"'))
(regex_cond_id <- capture(one_or_more(or(WRD, DOLLAR))) %R%
...
ui_inpt_template <- "tags$strong('${var}'), verbatimTextOutput('${var}Print'),"
ui_expr_template <- "tags$strong('${var}'), verbatimTextOutput('${var}Print'),"
